...
class Guarana(Plant) :
    ...
    def __init__(self, world, x = None, y = None) :
        if x == None and y == None :
            super().__init__(Species.GUARANA, 0, 0, 0, world, 1)
            width = world.width
            height = world.height
        ...
        super().__init__(Species.GUARANA, x, y, 0, world, 1)

    def action(self) :
        return super().action()

    def collision(self, other):
        self._world.report(f"{other.species.display_name()} ate Guarana and got its strength incremented by 3!")
        self.mark_dead()
        self._world.set_organism(None, self.x, self.y)
        other.move_organism(self.x, self.y)
        other.increment_strength(3)

    def draw_color(self):
        return self.species.color
    
    def sow(self, x, y):
        return Guarana(self._world, x, y)
